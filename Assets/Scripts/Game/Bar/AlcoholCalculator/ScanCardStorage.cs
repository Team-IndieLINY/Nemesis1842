...
public class ScanCardStorage : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void InsertScanCard(ScanCard scanCard)
    {
        ...
        UpdateAlcoholCalculator();
    }

    private void UpdateAlcoholCalculator()
    {
        int totalMaxAlcohol = 0;
        int totalMinAlcohol = 0;
        ...
        for (int i = 0; i < _cardSlots.Length; i++)
        {
            ...
            totalMaxAlcohol += _cardSlots[i].ScanCard.ScanData.MaxAlcohol;
            totalMinAlcohol += _cardSlots[i].ScanCard.ScanData.MinAlcohol;
        }

        _alcoholCalculator.UpdateAlcoholGaugeBar(totalMaxAlcohol, totalMinAlcohol);
    }
